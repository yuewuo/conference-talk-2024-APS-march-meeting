...
let segmented_untight_opacity = 0.4
...
export default {
    ...
    methods: {
        ...
        calculate_edge_to_dual_indices(snapshot) {
        ...
        get_interpolated_snapshot() {
            ...
            const dual_nodes = []
            if (snapshot_1.dual_nodes.length <= snapshot_2.dual_nodes.length) {
                ...
                // console.error("snapshot_1.dual_nodes.length <= snapshot_2.dual_nodes.length")
                for (let node_index = 0; node_index < snapshot_2.dual_nodes.length; ++node_index) {
                    dual_nodes.push(snapshot_2.dual_nodes[node_index])
                }
            }
        ...
        refresh_snapshot_data() {
            ...
            if (segmented.value) {
                edge_to_dual_indices = this.calculate_edge_to_dual_indices(snapshot)
            }
            ...
            for (let [i, edge] of snapshot.edges.entries()) {
                ...
                if (segmented.value) {
                    edge_branch_segmented_data = this.calculate_edge_branch_segmented(snapshot, edge_to_dual_indices, i)
                }
                for (let j = 0; j < edge.v.length; ++j) {
                    const vertex_index = edge.v[j]
                    const vertex_position = mwpf_data.positions[vertex_index]
                    ...
                    if (segmented.value) {
                        ...
                        for (const [node_index, accumulated_ratio, segment_ratio] of segments) {
                            ...
                            if (edge.v.length != 1) {
                                ...
                                let func = (ratio) => 0.5 * outline_ratio + (1 - 0.5 * outline_ratio) * ratio
         ...
         */
        calculate_edge_branch_segmented(snapshot, edge_to_dual_indices, edge_index) {
